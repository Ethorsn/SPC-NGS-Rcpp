...
rowvec TransformObsWishart(colvec X_t, int Ind, colvec Mu0, mat Sigma0){
...
// [[Rcpp::export]]
rowvec SimulateARL0Sigma(SEXP n, SEXP h, SEXP k, SEXP mu0, SEXP Sigma0, SEXP No_threads) {
  /* Function which simulates the in control average run length (ARL0) for covariance under a given control limit, allowance constant
   ...
   * 
   * Returns:
   *     Returns a vector of length n which contains simulated values of the average run length for the covariance .
   */
  ...
  try{
    ...
    {
      cout << "Inits done! Starting simulations. \n" << endl;
    }
...
#pragma omp parallel for num_threads(Nthread) shared(VecReturn) private(l, counter, Cstat, S_old, Sims,TransformObs, IamMax) firstprivate(H, K, Mu, Sigma,UnitMatrix,ZeroMean)
  for (l = 0; l < N;++l){
    // Inits for a run
    counter = 1;
    Cstat = 0;
...
/***R
#SimulateARL0Sigma(n=10, h=10, k=0.1, mu0=rep(0,2), Sigma0=diag(2), No_threads=5) 
*/