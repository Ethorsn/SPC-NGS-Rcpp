...
\begin{frame}\frametitle{Read level - Spearman correlation matrix}
 <<ReadlvlCor, fig.cap="Spearman correlation matrix of HiSeq 6 Read level measurements, paired end cycle=126">>=
...
<<Scenario1mean, fig.height=4, fig.cap="Scenario 1. MCUSUM model for different shifts. ">>=
 library(ggplot2)
 library(gridExtra)
...
<<Scenario2mean, fig.height=4, fig.cap="Scenario 2. MCUSUM model for different shifts.">>=
library(ggplot2)
library(gridExtra)
...
<<Tableprep>>=
#library(xtable)
Scenario1 <- data.frame("delta"=round(changeVector.case1[floor(seq(1,50,length=10))],4), "D"=round(Case1.CPD,4))

Scenario2 <- data.frame("delta"=round(changeVector.case2,4), "D"=round(Case2.CPD,4))
@
\begin{frame}\frametitle{CPD - simulations}
<<CPDfig, fig.height=4>>=
p1 <- ggplot(Scenario1) +
  geom_line(aes(x=delta,y=D)) +
  theme_bw() +
  xlab(expression(delta)) +
  ggtitle("Scenario 1")
p2 <- ggplot(Scenario2) +
...
\begin{frame}\frametitle{Hotelling's $T^2$}
 <<HotellingsPicture, fig.cap="Hotellings T2 statistic for HiSeq 3,4 and 5. HiSeq 3 has no paired end cycle=126 runs!", fig.height=4>>=
 p1 <- ggplot() +
